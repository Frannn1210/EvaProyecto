# UNIVERSIDAD DE INGENIERÍA Y TECNOLOGÍA

## Informe Final

### AUTOR(ES)

- Campos Dominguez Fatima Guadalupe Eumelia 100%
- Huerta Roque Francis Andres 100%

### DOCENTE

Maria Cristina Ramos Toledo

---

## VitalLunch

### 1. Problema y Oportunidad

De acuerdo con la Encuesta Demográfica y de Salud Familiar (ENDES) 2023 del Instituto Nacional de Estadística e Informática (INEI), el 69.4% de la población adulta en el Perú presenta exceso de peso. Esta situación se agrava en el entorno laboral de Lima Metropolitana, donde según la Asociación Peruana de Delivery (APED, 2023), el 68% de los limeños utiliza servicios de delivery de alimentos al menos una vez por semana, predominando opciones poco saludables.

La oportunidad se sustenta en el crecimiento del sector de alimentos saludables, que según el Ministerio de la Producción (PRODUCE, 2023), ha registrado una tasa promedio anual del 15% en el último quinquenio. Adicionalmente, la alta concentración de establecimientos empresariales en distritos como Miraflores, San Isidro y Surco reportada por la SUNAT configura un mercado objetivo sustancial y desatendido.

### 2. Objetivos

2.1 Objetivo General: Implementar un servicio de delivery de almuerzos saludables mediante aplicación móvil dirigido a empresas de Miraflores, San Isidro y Surco.

2.2 Objetivos Específicos:

- Desarrollar una plataforma tecnológica nativa para gestión de pedidos
- Establecer una cocina central con certificación DIGESA
- Captar empresas clientes mediante estrategias B2B
- Alcanzar indicadores de rentabilidad financiera positivos

### 3. Justificación

3.1 Técnica
Hoy no existe una solución pensada para oficinas que garantice comer saludable y a tiempo con una experiencia simple. Las apps actuales muestran muchas opciones, pero no aseguran información clara del menú, personalizaciones básicas, pago dentro de la misma app y entrega en una ventana de hora definida. Como emprendimiento, proponemos empezar con una app ligera que haga bien lo esencial: mostrar el menú con datos simples, permitir ajustes como vegetariano o sin gluten, cobrar de forma segura y avisar el estado del pedido con una hora estimada. También falta una forma sencilla para que las empresas aporten una parte del costo del almuerzo, lo cual ayuda a que más personas pidan sin que sea caro. Nuestra solución integra ese aporte de manera clara y automática. En la logística, vemos que no se agrupan bien los pedidos por edificio o zona en las horas pico, lo que retrasa y enfría la comida. Nosotros organizamos rutas por zonas y entregas por ventanas para llegar a tiempo y mantener temperatura. Además, casi nadie mide lo básico para mejorar. Tomaremos datos simples como entregas a tiempo, temperatura al llegar, problemas reportados y una encuesta corta. Con esa información corregimos rápido, sin complicar el sistema ni subir costos.

3.2 Económica
Para muchos trabajadores, comer rápido hoy termina siendo más caro de lo que parece cuando se suman menor energía en la tarde y más ausencias por mala alimentación. Los precios del delivery también suben por compras sin plan y repartos poco eficientes. Como emprendimiento, vamos a cocinar por lotes según la demanda confirmada para comprar mejor y reducir desperdicio. También vamos a juntar entregas por empresa o zona para que un repartidor deje varios pedidos en el mismo lugar, bajando tiempo y costo por pedido. Cuando la empresa decide aportar una parte del precio, el usuario ve un monto final más accesible y pide con más frecuencia, lo que vuelve la demanda más estable y permite planificar compras y reparto sin sorpresas. Con más pedidos por parada, compras planificadas y un precio final conveniente, podemos llegar a la meta de ventas del primer año manteniendo márgenes sanos. En simple: vendemos de forma ordenada, gastamos menos en reparto e insumos y el usuario paga un precio que le hace sentido, mientras validamos el modelo y crecemos paso a paso.

---

## 4. Mercado

4.1 Breve análisis del mercado
Lima Metropolitana concentra 9,674,000 habitantes (INEI, 2024) y es la zona con mayor densidad poblacional y actividad económica del país. Nuestros distritos objetivo —Miraflores (99,223 hab.), San Isidro (68,438 hab.) y Surco (94,014 hab.) según INEI 2023— reúnen gran cantidad de oficinas y poder adquisitivo. El sector de alimentos representa el 28.7% de la producción manufacturera nacional (PRODUCE), lo que asegura oferta de insumos y proveedores. El uso de apps de reparto está muy extendido: la APED (2023) señala que el 68% de limeños pide delivery al menos una vez por semana y que los pedidos "saludables" vienen creciendo. Además, hay base empresarial suficiente: SUNAT registra 286,518 establecimientos comerciales en Lima Metropolitana. A nivel de hábitos, 8 de cada 10 peruanos quieren comer más sano, aunque solo 34% lo hace de forma constante; esto muestra una brecha clara entre intención y práctica que nuestro servicio puede cerrar con opciones saludables, puntuales y fáciles de pedir por app.

4.2 Estimación de Demanda
Con 80 empresas y 50 personas por empresa, esperamos que el 15% de los colaboradores use el servicio y pida 1.25 veces por semana. Eso nos da 30,000 pedidos al año. Con un precio promedio de S/ 22 por pedido, las ventas anuales serían S/ 660,000, con un ritmo de 150 pedidos diarios en promedio. Cada usuario activo haría ≈50 pedidos al año (1.25 × 40 semanas), lo que equivale a ≈S/ 1,100 por persona al año.

---

## 5. Recursos Clave

### 5.1 Humanos

- 1 Jefe/a de cocina: responsable de producción, calidad y costos; coordina compras y turnos.
- 2 Cocineros/as: ejecutan recetas y porcionado según plan del día.
- 2 Apoyos de pre-preparación y empaque: armado y sellado
- 1 Coordinador/a de rutas y calidad: arma rutas por ventanas, controla temperatura/salida y tiempos de entrega.
- 1 Atención al cliente (part-time): WhatsApp/app, confirmaciones y registro de incidencias.
- 1 Nutricionista (por horas): diseño de menús, calorías/alérgenos y validación de sustituciones.
- 1 Desarrollador/a full-stack con UI/UX (freelance): responsable del desarrollo y mantenimiento de la aplicación.
- 1 Agente de marketing/comunicación (part-time o freelance): encargado/a de hacer la "propaganda" del servicio. Maneja redes sociales

### 5.2 Tecnología

- Aplicación móvil / web: plataforma donde los usuarios ven el menú, hacen sus pedidos, pagan con Yape o tarjeta y reciben el estado de su pedido en tiempo real.
- Sistema en la nube: base de datos sencilla que guarda usuarios, menús y pedidos; con copias de seguridad diarias para no perder información.
- Panel administrativo: espacio interno para que el equipo vea los pedidos del día, controle entregas y genere reportes simples; se apoya en herramientas fáciles como Google Sheets o HubSpot gratuito.
- Soporte operativo: WhatsApp Business para atención al cliente y Google Sheets como respaldo en caso la app falle.

### 5.3 Infraestructura

- Cocina central alquilada (80–100 m²): con zonas básicas: recibo de insumos, almacén seco y frío, área para picar/preparar, cocción, empaque y despacho.
- Equipo esencial: horno, ollas grandes, mesas de acero, selladora, balanza, contenedores térmicos para delivery y una refri/cámara para conservar.
- Delivery: tercerizado con motos de 1–2 operadores por cada ventana de entrega; se agrupan pedidos por zona para ganar tiempo.
- Administración: cowork o oficina pequeña para coordinar operaciones y ventas; RUC activo y facturación electrónica (SUNAT).

---

## 6. Flujo de Caja Económico y Financiero

### 6.1 Horizonte del Proyecto (5 años)

| Año   | Enfoque                  | Metas                                                                                                                                           |
| ----- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Año 1 | Lanzamiento y validación | Poner en marcha la app, atender en Miraflores, San Isidro y Surco; lograr 80 empresas clientes y 30,000 pedidos en el año.                      |
| Año 2 | Optimización             | Mejorar procesos de cocina y delivery, reducir tiempos de entrega y asegurar calidad constante.                                                 |
| Año 3 | Estabilidad financiera   | Alcanzar punto de equilibrio, asegurar flujo de caja positivo y mantener satisfacción del cliente.                                              |
| Año 4 | Mejora continua          | Fortalecer la app con mejoras sencillas (como historial de pedidos o encuestas de satisfacción) y aumentar pedidos por cliente.                 |
| Año 5 | Consolidación            | Ser reconocidos en Lima como un emprendimiento confiable de almuerzos saludables para oficinas, manteniendo crecimiento estable y rentabilidad. |

### 6.2 Supuestos Clave Utilizados

#### Demanda y Precio

Operaremos con 80 empresas y 50 personas por empresa (total 4,000 elegibles). Suponemos adopción 15% (≈600 usuarios que compran) y frecuencia 1.25 pedidos por semana durante 40 semanas activas al año (5 días laborales). El precio al cliente es S/ 22 por almuerzo y se mantiene fijo en S/ 22 durante los 5 años. Desde el Año 2 puede considerarse crecimiento de pedidos 5% anual, sin cambiar el precio.

#### Costos Variables por Pedido

El costo variable unitario se compone de: ingredientes S/ 11.80, empaque S/ 1.40, delivery tercerizado S/ 2.80, comisiones 4.5% del ticket (con S/ 22 ≈ S/ 1.00 por pedido) y otros S/ 0.50 (bolsas, mermas, servilletas). Para los años siguientes aplica inflación 3% anual sobre ingredientes y empaque. El delivery se mantiene en S/ 2.80 en A1–A2 y podría subir S/ 0.10 por año desde A3 si fuese necesario. Las comisiones se mantienen en 4.5% del mismo ticket S/ 22.

#### Costos Fijos Mensuales

- Alquiler de cocina (80–100 m²): S/ 5,500.
- Servicios (agua, luz, gas, internet): S/ 1,200.
- Planilla base: Jefe/a de cocina S/ 3,200; 2 cocineros/as S/ 1,800 c/u (S/ 3,600); 2 apoyos pre-prep/empaque S/ 1,200 c/u (S/ 2,400); coordinador/a de rutas y calidad S/ 1,800; atención al cliente part-time S/ 800.
- Nutricionista por horas: S/ 800.
- Marketing (redes/ads): S/ 1,200.
- Nube y software: S/ 500.
- Seguros/otros: S/ 300.

Desde el Año 2 aplica inflación 3% anual a alquiler, servicios y sueldos. Marketing y nube pueden mantenerse en A1–A2 y crecer 5% anual desde A3 si hay retorno.

#### Inversión Inicial

- Equipos de cocina esenciales: S/ 55,000 (horno/conv., marmita/ollas, mesas inox, selladora)
- Adecuaciones/mobiliario: S/ 10,000
- Contenedores isotérmicos y utensilios: S/ 7,000
- IT menor: S/ 5,000 (PC, 2 tablets, impresora térmica, balanzas)
- MVP de software (app + backend + panel): S/ 18,000
- Marca/diseño: S/ 1,000
- Licencias y permisos: S/ 2,000
- Imprevistos: S/ 6,000
- Capital de trabajo inicial: S/ 20,000 (inventario, garantías, caja mínima)

**Depreciación/amortización de referencia:** 5 años (equipos/adecuaciones/utensilios/IT) y 3 años (software); marca 10 años si la incluyes. Valores residuales a A5: equipos 20%, utensilios/IT 10%; adecuaciones/software/marca 0%. Recuperación de capital de trabajo: 100% al final.

#### Capital de Trabajo y Políticas Operativas

- Cobro inmediato en la app (cuentas por cobrar = 0 días)
- Inventario promedio 7 días (rotación semanal)
- Pagos a proveedores 15 días
- La caja mínima está incluida dentro del capital de trabajo inicial S/ 20,000

#### Impuestos y Tasas Financieras

- Impuesto a la Renta: 29.5%
- Inflación: 3% anual (para actualizar costos)
- Tasa de descuento (VAN/TIR): 11%

#### Deuda

Si se decide financiar una parte: préstamo S/ 70,000, tasa 18% anual, plazo 36 meses (sistema francés), comisión inicial 1%. Escudo fiscal = intereses × 29.5%. Si no se usará deuda, fijar estos valores en 0.

### 6.3 Flujo de Caja Operativo (Costos e Ingresos Estimados)

| Año | Pedidos | Ventas (S/.)    | Costos Variables (S/.) | Costos Fijos (S/.) | EBIT (S/.)    |
| --- | ------- | --------------- | ---------------------- | ------------------ | ------------- |
| 1   | 30000   | S/ 780,000.00   | 525000                 | 255600             | S/ -600.00    |
| 2   | 39000   | S/ 1,044,420.00 | 702000                 | 263268             | S/ 79,152.00  |
| 3   | 50700   | S/ 1,398,478.38 | 940485                 | 271166.04          | S/ 186,827.34 |
| 4   | 65910   | S/ 1,872,562.55 | 1260199.2              | 279301.0212        | S/ 333,062.33 |
| 5   | 85683   | S/ 2,507,361.26 | 1688811.93             | 287680.0518        | S/ 530,869.27 |

### 6.4 Flujo de caja económico

| Año | Utilidad Operativa (S/.) | Inversión (S/.) | Valor Residual (S/.) | Flujo Económico |
| --- | ------------------------ | --------------- | -------------------- | --------------- |
| 0   |                          | -124000         |                      | -124000         |
| 1   | -S/.600.00               |                 |                      | -S/.600.00      |
| 2   | S/.79,152.00             |                 |                      | S/.79,152.00    |
| 3   | S/.186,827.34            |                 |                      | S/.186,827.34   |
| 4   | S/.333,062.33            |                 |                      | S/.333,062.33   |
| 5   | S/.567,869.27            |                 | 37000                | S/.567,869.27   |

### 6.5 Flujo de Caja de la Deuda

| Año | Préstamo Recibido | Amortización Capital | Intereses | Servicio Deuda Total (-) | Escudo Fiscal (29.5%) |
| --- | ----------------- | -------------------- | --------- | ------------------------ | --------------------- |
| 0   | 70000             |                      |           | -700                     |                       |
| 1   |                   | 17020                | 12,560    | -29580                   | 3707                  |
| 2   |                   | 20086                | 9,494     | -29580                   | 2800                  |
| 3   |                   | 23704                | 5,876     | -29580                   | 1734                  |
| 4   |                   |                      |           |                          |                       |
| 5   |                   |                      |           |                          |                       |

### 6.6 Flujo de Caja Financiero

| Año | Flujo Económico | Préstamo (+) | Servicio Deuda (-) | Flujo Financiero |
| --- | --------------- | ------------ | ------------------ | ---------------- |
| 0   | -124000         | 70000        | -700               | -54700           |
| 1   | -S/ 600.00      |              | -29,580            | -S/ 30,180.00    |
| 2   | S/ 79,152.00    |              | -29,580            | S/ 49,572.00     |
| 3   | S/ 186,827.34   |              | -29,580            | S/ 157,247.34    |
| 4   | S/ 333,062.33   |              |                    | S/ 333,062.33    |
| 5   | S/ 567,869.27   |              |                    | S/ 567,869.27    |

---

## 7. Evaluación económica y financiera

### 7.1 Costo del capital (COK y WACC)

Para evaluar si el proyecto vale la pena, necesitamos definir cuánto nos cuesta el dinero. Los accionistas esperan ganar al menos 11% (COK), que es lo que podrían obtener en otra inversión de riesgo similar. Como vamos a tomar un préstamo de S/ 70,000 al 18% anual y los accionistas aportan S/ 54,700, el costo promedio ponderado del capital (WACC) queda cerca de 12%. Este 12% será nuestra tasa mínima de referencia para saber si los flujos del proyecto cubren lo que esperan tanto el banco como los inversionistas.

| Parámetro                              | Valor     |
| -------------------------------------- | --------- |
| COK (ke)                               | 11%       |
| Deuda (D)                              | S/ 70,000 |
| Patrimonio (E)                         | S/ 54,700 |
| Tasa de impuesto                       | 29.5%     |
| Costo de la deuda después de impuestos | 12.69%    |
| WACC estimado                          | ≈ 12.0%   |

### 7.2 Cálculo de indicadores

#### 7.2.1 Económico (flujo económico, descuento WACC ≈ 12%)

Cuando calculamos el VAN económico (sin considerar la deuda), el proyecto genera un valor presente de S/ 605,652 con una TIR del 77%. Eso significa que por cada sol que invertimos, recuperamos casi 6 veces su valor en términos actuales. El periodo de recuperación es de 2.45 años, lo cual es bastante rápido para un emprendimiento de este tipo.

| Indicador                    | Resultado    |
| ---------------------------- | ------------ |
| VAN (12%)                    | ≈ S/ 605,652 |
| TIR                          | ≈ 77%        |
| PRD simple                   | ≈ 2.24 años  |
| PRD descontado (12%)         | ≈ 2.45 años  |
| IR (PV influjos / Inversión) | ≈ 5.89       |

#### 7.2.2 Financiero (flujo al accionista, descuento COK 11%)

Desde la perspectiva del accionista (con deuda incluida), los números mejoran: el VAN financiero sube a S/ 629,707 y la TIR se dispara a 100%. Esto pasa porque la deuda nos ayuda a apalancar la inversión inicial y los intereses generan escudo fiscal. Para los socios, recuperar su aporte toma 2.36 años y cada sol que ponen genera más de 12 veces su valor presente.

| Indicador                   | Resultado    |
| --------------------------- | ------------ |
| VAN (11%)                   | ≈ S/ 629,707 |
| TIR                         | ≈ 100%       |
| PRD simple                  | ≈ 2.22 años  |
| PRD descontado (11%)        | ≈ 2.36 años  |
| IR (PV influjos / Aporte E) | ≈ 12.51      |

### 7.3 Interpretación de resultados

Los números son claros: el proyecto funciona. Tanto el VAN económico como el financiero son bastante positivos, y la TIR supera por mucho lo que esperan los inversionistas (11%) y el costo promedio del capital (12%). Recuperamos la inversión antes del tercer año, lo que reduce el riesgo. Ahora bien, estos resultados dependen de cumplir con la demanda proyectada de 30,000 pedidos en el primer año y mantener los costos bajo control. Si vendemos menos o se nos disparan los gastos, los márgenes se aprietan. Por eso es clave validar rápido con pilotos y tener disciplina financiera desde el día uno.

---

## 8. Análisis de riesgos

### 8.1 Riesgos operativos

| Riesgo                             | Probabilidad | Impacto | Mitigación                                                                                      |
| ---------------------------------- | ------------ | ------- | ----------------------------------------------------------------------------------------------- |
| Demanda menor a la prevista        | Media        | Alta    | Pilotos por empresa, campañas B2B y subsidio corporativo; ajuste de producción por lotes.       |
| Retrasos en entrega (rutas pico)   | Alta         | Alta    | Ventanas de entrega, ruteo por zonas, buffers de tiempo, seguimiento en tiempo real.            |
| Pérdida de temperatura/calidad     | Media        | Alta    | Contenedores térmicos, control de temperatura de salida/llegada, estandarización de porcionado. |
| Fallas de la app o pagos           | Media        | Media   | Soporte WhatsApp Business y plan de contingencia en Google Sheets; pasarelas redundantes.       |
| Ruptura de stock/proveedores       | Baja         | Media   | Doble proveedor crítico, compras planificadas, inventario mínimo de seguridad (7 días).         |
| Inocuidad / incumplimientos DIGESA | Baja         | Alta    | POES, BPM, registros de trazabilidad y auditorías internas periódicas.                          |
| Rotación de personal clave         | Media        | Media   | Manuales operativos, capacitación cruzada y estructura de turnos con respaldo.                  |

### 8.2 Riesgos financieros

| Riesgo                                          | Probabilidad | Impacto | Mitigación                                                                                         |
| ----------------------------------------------- | ------------ | ------- | -------------------------------------------------------------------------------------------------- |
| Incremento de costos de insumos (>3% inflación) | Media        | Alta    | Contratos marco, sustituciones de menú, revisión trimestral de precios con empresas.               |
| Aumento de comisiones y delivery                | Media        | Media   | Negociación con agregadores, mayor uso de flota tercerizada por zona, incentivos a pedido directo. |
| Liquidez insuficiente en arranque               | Media        | Alta    | Capital de trabajo suficiente, control de gastos, línea de crédito pequeña para contingencias.     |
| Riesgo de deuda (tasa 18% y servicio)           | Baja         | Media   | Mantener servicio en sistema francés estable; amortización anticipada si flujo excede meta.        |
| Tipo de cambio en insumos importados            | Baja         | Media   | Coberturas simples (stock estratégico), compras locales equivalentes.                              |
| Caída en frecuencia de uso                      | Media        | Alta    | Programa de fidelización, menús rotativos, subsidios corporativos parciales.                       |

---

## 9. Análisis de sensibilidad

### 9.1 Variable 1: Precio de venta (±10%)

Aquí probamos qué pasa si el precio cambia. Si por presión de la competencia o negociación con empresas terminamos cobrando S/ 19.80 en vez de S/ 22, el VAN baja casi 40% pero se mantiene positivo (S/ 387,450). Si logramos cobrar S/ 24.20 porque el servicio vale la pena, el VAN sube a S/ 871,964. Claramente el precio es clave para la rentabilidad.

| Variación Precio | Precio (S/) | VAN Financiero (aprox.) | Cambio vs. Base |
| ---------------- | ----------- | ----------------------- | --------------- |
| -10%             | S/ 19.80    | S/ 387,450              | -38.5%          |
| Base             | S/ 22.00    | S/ 629,707              | 0%              |
| +10%             | S/ 24.20    | S/ 871,964              | +38.5%          |

**¿Qué significa esto?** Que no podemos bajar mucho el precio sin afectar seriamente las ganancias. Por eso es importante cerrar bien los subsidios corporativos desde el inicio y demostrar que el servicio vale lo que cuesta.

### 9.2 Variable 2: Costos variables unitarios (±10%)

Ahora vemos qué pasa si los costos de ingredientes, empaque o delivery suben o bajan. Si negociamos bien y bajamos los costos variables a S/ 15.75 por pedido, el VAN sube 26% (S/ 792,180). Si por inflación o mala negociación los costos se van a S/ 19.25, el VAN cae a S/ 467,234, que sigue siendo positivo pero más ajustado.

| Variación Costos | Costo Variable Base | VAN Financiero (aprox.) | Cambio vs. Base |
| ---------------- | ------------------- | ----------------------- | --------------- |
| -10%             | S/ 15.75            | S/ 792,180              | +25.8%          |
| Base             | S/ 17.50            | S/ 629,707              | 0%              |
| +10%             | S/ 19.25            | S/ 467,234              | -25.8%          |

**¿Qué significa esto?** Que tenemos que cuidar mucho los costos operativos. Cada sol que ahorremos por pedido se multiplica por miles de pedidos al año.

### 9.3 Conclusión del análisis de sensibilidad

Aunque el proyecto aguanta variaciones de precio y costos, no hay mucho margen para relajarse. Lo más importante es:

- Cerrar contratos con empresas que garanticen el precio de S/ 22 o cercano (con subsidio incluido).
- Negociar con proveedores desde el inicio para tener costos predecibles y ojalá descuentos por volumen.
- Revisar cada mes el margen unitario (precio menos costos variables) y actuar rápido si baja de S/ 4.50 por pedido.

---

## 10. Análisis de escenarios

### 10.1 Definición de escenarios

Se analizan tres escenarios combinando las siguientes variables clave:

| Variable                        | Optimista     | Base   | Pesimista      |
| ------------------------------- | ------------- | ------ | -------------- |
| Precio de venta (S/)            | 24.00 (+9%)   | 22.00  | 20.00 (-9%)    |
| Costos variables unitarios (S/) | 16.00 (-8.6%) | 17.50  | 19.50 (+11.4%) |
| Demanda anual (pedidos A1)      | 33,000 (+10%) | 30,000 | 24,000 (-20%)  |

### 10.2 Escenario Optimista

**Supuestos:**

- Precio S/ 24 (empresas aceptan subsidio menor y usuarios pagan más por calidad)
- Costos variables S/ 16 (negociación exitosa con proveedores, economías de escala)
- Demanda +10% (adopción 16.5% en lugar de 15%)

**Resultados:**

| Indicador            | Valor        |
| -------------------- | ------------ |
| VAN Financiero (11%) | ≈ S/ 985,420 |
| TIR Financiero       | ≈ 145%       |
| PRD descontado       | ≈ 1.9 años   |

**Interpretación:** Rentabilidad muy elevada, TIR supera ampliamente el COK. Proyecto altamente atractivo con márgenes robustos.

### 10.3 Escenario Base

**Supuestos:**

- Precio S/ 22 (supuesto inicial)
- Costos variables S/ 17.50 (según estructura presentada)
- Demanda 30,000 pedidos (adopción 15%)

**Resultados:**

| Indicador            | Valor        |
| -------------------- | ------------ |
| VAN Financiero (11%) | ≈ S/ 629,707 |
| TIR Financiero       | ≈ 100%       |
| PRD descontado       | ≈ 2.36 años  |

**Interpretación:** Proyecto viable con indicadores positivos. Representa el escenario esperado con supuestos conservadores.

### 10.4 Escenario Pesimista

**Supuestos:**

- Precio S/ 20 (presión competitiva o menor subsidio corporativo)
- Costos variables S/ 19.50 (inflación mayor, proveedores con menor poder de negociación)
- Demanda -20% (adopción 12%, resistencia al cambio, competencia)

**Resultados:**

| Indicador            | Valor        |
| -------------------- | ------------ |
| VAN Financiero (11%) | ≈ S/ 187,340 |
| TIR Financiero       | ≈ 48%        |
| PRD descontado       | ≈ 3.2 años   |

**Interpretación:** Proyecto aún viable pero con márgenes ajustados. VAN positivo y TIR superior al COK (11%), aunque la recuperación es más lenta.

### 10.5 Comparación de escenarios

| Escenario | VAN Financiero | TIR  | Variación VAN vs. Base |
| --------- | -------------- | ---- | ---------------------- |
| Optimista | S/ 985,420     | 145% | +56.5%                 |
| Base      | S/ 629,707     | 100% | 0%                     |
| Pesimista | S/ 187,340     | 48%  | -70.2%                 |

**Gráfico conceptual de sensibilidad:**

- Todos los escenarios mantienen VAN > 0 y TIR > COK (11%).
- El rango de VAN va desde S/ 187k (pesimista) hasta S/ 985k (optimista).
- Incluso en el peor caso, el proyecto genera valor para los accionistas.

### 10.6 Conclusión del análisis de escenarios

Lo que sacamos en limpio es que VitalLunch funciona incluso cuando las cosas no salen perfectas. Aunque en el escenario pesimista bajemos precio, suban costos y vendamos 20% menos, el VAN sigue positivo (S/ 187k) y la TIR (48%) supera lo que piden los inversionistas (11%). Eso nos da tranquilidad.

Ahora, lo que más mueve la aguja son el precio y la demanda. Si logramos mantener el precio en S/ 22 (con ayuda de subsidios corporativos) y conseguimos que la gente pida con frecuencia, el proyecto despega. Los costos variables también importan, pero tienen menos peso que las ventas.

Entonces, ¿dónde hay que poner energía?

- **Primero:** asegurar que las empresas den subsidio y que el precio final sea atractivo para los usuarios.
- **Segundo:** hacer campaña fuerte los primeros 6 meses para que la gente pruebe el servicio y se enganche.
- **Tercero:** negociar bien con proveedores desde el inicio para no llevarnos sorpresas con los costos.

En resumen: el proyecto aguanta golpes, pero hay que trabajar bien los tres puntos anteriores para no quedarnos en el escenario pesimista.

---

## 11. Conclusiones y Recomendaciones

### 11.1 Viabilidad del Proyecto

Después de revisar todos los números, podemos decir con confianza que **VitalLunch es un proyecto viable y rentable**. Veamos por qué:

**Desde el punto de vista económico (sin considerar la deuda):**

El VAN económico sale en S/ 605,652 con un WACC del 12%. Eso quiere decir que el proyecto genera valor muy por encima de lo que cuesta el capital. La TIR económica es del 77%, lo cual es bastante alto — básicamente significa que el negocio devuelve la inversión y además genera mucha ganancia. Recuperamos el dinero invertido en 2.45 años, que es menos de la mitad del horizonte de 5 años. Y el índice de rentabilidad (5.89) nos dice que por cada sol que metemos, sacamos casi 6 soles de vuelta en términos de valor presente.

**Desde el punto de vista del accionista (con deuda incluida):**

Aquí los números mejoran todavía más. El VAN financiero sube a S/ 629,707 porque la deuda nos ayuda a apalancar (el escudo fiscal de los intereses suma). La TIR financiera se va hasta 100%, lo que refleja que el aporte de los socios se multiplica rápido. Recuperamos la inversión en 2.36 años, y cada sol que los accionistas ponen genera S/ 12.51 en valor presente. La deuda, bien usada, nos ayuda bastante.

**¿Qué tan sólido es el proyecto?**

Probamos diferentes escenarios y el proyecto aguanta bien. Incluso si el precio baja 10% o los costos suben 10%, el VAN se mantiene positivo. En el escenario más pesimista (precio bajo, costos altos, menos demanda), el VAN sigue siendo S/ 187,340 con una TIR del 48%, que todavía supera el COK de 11%. Esto nos da seguridad de que no estamos trabajando con un modelo frágil.

**En resumen:** VitalLunch cumple con lo que buscamos en un proyecto de inversión. Los números son buenos, el modelo aguanta golpes y la deuda nos ayuda a potenciar los retornos. Ahora hay que ejecutar bien para que las proyecciones se hagan realidad.

### 11.2 Riesgos Identificados

Como cualquier emprendimiento, VitalLunch tiene riesgos. Identificamos **trece riesgos principales** que hay que vigilar, divididos entre operativos y financieros:

**Los tres riesgos operativos más críticos:**

1. **Que la demanda sea menor a lo esperado:** Si no llegamos al 15% de adopción o la gente pide menos de 1.25 veces por semana, los ingresos se caen. Para evitarlo vamos a hacer pilotos con algunas empresas antes de lanzar masivamente, mover campañas B2B fuertes y asegurar subsidios corporativos que hagan atractivo el precio. Si vemos que la demanda flaquea, ajustamos producción rápido para no generar desperdicios.

2. **Retrasos en las entregas:** Si llegamos tarde en hora pico, la gente se molesta y no vuelve a pedir. Por eso vamos a trabajar con ventanas de entrega claras, armar rutas por zonas para que los repartidores no den vueltas de más, y darles un buffer de tiempo. Además, vamos a tener seguimiento en tiempo real para saber dónde está cada pedido.

3. **Que la comida llegue fría o mal empacada:** Si el almuerzo llega tibio o se voltea en el camino, perdemos credibilidad. Acá la solución es usar contenedores térmicos buenos, controlar la temperatura al salir y al llegar, y estandarizar cómo porcionamos y empacamos todo.

**Los tres riesgos financieros más críticos:**

4. **Que los costos de ingredientes se disparen:** Si la inflación supera el 3% que proyectamos, los márgenes se aprietan rápido. Para cubrirn os, vamos a firmar contratos marco con proveedores (con precios fijos o topes), tener sustituciones de menú preparadas si algo se pone muy caro, y revisar precios con las empresas cada tres meses si es necesario.

5. **Quedarnos sin caja en el arranque:** El primer año es ajustado y si no controlamos bien los gastos podemos quedarnos sin liquidez. Por eso separamos S/ 20,000 de capital de trabajo inicial, vamos a llevar control estricto de cada gasto y tenemos pensado una línea de crédito pequeña de emergencia por si acaso.

6. **Que la gente pida cada vez menos:** Si el menú se vuelve aburrido o aparece competencia más atractiva, la frecuencia de uso baja y eso mata las ventas. Vamos a rotar menús seguido, armar un programa de fidelización (tipo "10° almuerzo gratis") y trabajar con las empresas para que mantengan o suban los subsidios.

**Otros riesgos que hay que monitorear:**

- Si la app o los pagos fallan, tenemos respaldo por WhatsApp y Google Sheets.
- Si un proveedor nos falla, tenemos doble proveedor para ingredientes críticos.
- Para cumplir con DIGESA, vamos a tener registros de todo y hacer auditorías internas.
- Si se va personal clave, tenemos manuales y capacitación cruzada para que otros cubran.
- Si suben comisiones de apps o delivery, negociamos con los agregadores o usamos flota propia por zonas.
- Si algún insumo importado se encarece por el dólar, compramos stock adelantado o buscamos alternativas locales.

**En resumen:** ninguno de estos riesgos es catastrófico si lo manejamos bien. Tenemos plan de mitigación para cada uno y vamos a monitorearlos mensualmente. Lo importante es actuar rápido cuando veamos señales de alerta.

### 11.3 Recomendaciones Finales

Para maximizar la probabilidad de éxito de VitalLunch y asegurar el cumplimiento de las proyecciones, se recomiendan las siguientes acciones estratégicas:

**Fase de Lanzamiento (Meses 1-6)**

1. **Validación rápida de demanda:** Ejecutar pilotos con 10-15 empresas piloto (pequeña muestra representativa) antes del lanzamiento masivo. Medir adopción real, frecuencia de pedidos y satisfacción en las primeras 4 semanas. Ajustar estrategia comercial si la adopción es <12%.

2. **Asegurar subsidios corporativos:** Negociar con las empresas cliente un subsidio de S/ 5-8 por almuerzo (30-40% del precio), posicionándolo como beneficio de bienestar. Esto reduce la barrera de precio para el usuario final y estabiliza la demanda.

3. **Inversión en marketing de contenido:** Crear campañas educativas sobre alimentación saludable, testimonios de usuarios y beneficios corporativos (productividad, retención de talento). Invertir al menos S/ 1,200 mensuales en redes sociales y publicidad digital segmentada.

4. **Control riguroso de calidad desde el día 1:** Implementar checklist de calidad en cada etapa (recepción, preparación, empaque, despacho). Registrar temperatura de cada lote despachado y tiempos de entrega. Establecer meta de 95% de entregas a tiempo y <2% de quejas por calidad en el primer trimestre.

**Gestión Operativa y Financiera (Año 1-2)**

5. **Monitoreo mensual de márgenes unitarios:** Calcular margen bruto por pedido (precio - costos variables) cada mes. Si el margen cae por debajo de S/ 4.50 por pedido, activar plan de contingencia: renegociar con proveedores, ajustar porciones o revisar precios con empresas.

6. **Optimización de costos variables:** Negociar contratos marco con proveedores de ingredientes clave (pollo, vegetales, granos) con volúmenes trimestrales. Buscar descuentos por volumen de 5-10% y cláusulas de ajuste de precio máximo trimestral.

7. **Capital de trabajo dinámico:** Mantener caja mínima de S/ 10,000 en los primeros 6 meses para cubrir imprevistos (pagos urgentes, reposición de equipos menores). Configurar línea de crédito revolvente de S/ 15,000 con banco para contingencias, a usar solo si los flujos caen 20% por debajo de proyecciones.

8. **Cumplimiento estricto del servicio de deuda:** Priorizar el pago de amortización e intereses en los primeros 36 meses (S/ 29,580 anuales). Considerar amortizaciones anticipadas parciales si el flujo de caja supera proyecciones en +15%, para reducir carga de intereses.

**Escalabilidad y Mejora Continua (Año 3-5)**

9. **Inversión en tecnología:** A partir del Año 3, destinar S/ 5,000-8,000 a mejoras de la app: historial de pedidos, recomendaciones personalizadas, encuestas de satisfacción automatizadas y programa de puntos. Esto puede aumentar frecuencia de uso en 10-15%.

10. **Diversificación de menú:** Introducir opciones premium (+S/ 4-6) o menús especiales (veganos, keto, sin gluten) para capturar segmentos adicionales y aumentar ticket promedio. Evaluar aceptación con encuestas trimestrales.

11. **Programa de fidelización:** Implementar incentivos como "10° almuerzo gratis" o descuentos progresivos por frecuencia (5% descuento si pides >3 veces/semana). Esto eleva la frecuencia y reduce sensibilidad al precio.

12. **Expansión geográfica controlada:** Si la demanda en Miraflores, San Isidro y Surco alcanza 40,000 pedidos/año (33% sobre meta base), evaluar expansión a La Molina o Lince con modelo probado, sin comprometer calidad ni liquidez.

**Gestión de Riesgos Continua**

13. **Tablero de indicadores clave (KPI dashboard):** Monitorear mensualmente: adopción %, frecuencia de pedido, % entregas a tiempo, margen bruto unitario, días de inventario, rotación de personal. Definir umbrales de alerta (ej. margen <S/ 4, entregas a tiempo <90%) para activar planes de acción inmediatos.

14. **Revisión trimestral de supuestos:** Cada 3 meses, contrastar proyecciones vs. resultados reales en demanda, costos variables y costos fijos. Ajustar el flujo de caja del año en curso si las desviaciones superan ±10%.

15. **Plan de contingencia ante caída de demanda:** Si la demanda del mes cae >20% respecto a proyección, activar: (a) campaña promocional agresiva (2x1, semana gratis), (b) encuestas para identificar causas (precio, calidad, competencia), (c) ajuste temporal de producción para evitar mermas.

**Gobernanza y Responsabilidad**

16. **Equipo de liderazgo con roles claros:** Designar un/a gerente general responsable de P&L (profit & loss), un/a jefe/a de operaciones (cocina, entrega, calidad) y un/a responsable comercial/marketing. Reuniones semanales de seguimiento en los primeros 6 meses.

17. **Auditorías de calidad externas:** Contratar auditoría semestral de inocuidad y BPM (Buenas Prácticas de Manufactura) por terceros certificados, para mantener estándares DIGESA y prevenir incidentes que puedan afectar reputación.

18. **Transparencia con inversionistas:** Reportar mensualmente a los accionistas: ingresos, costos, flujo de caja acumulado, VAN actualizado y estado de riesgos. Esto facilita decisiones rápidas ante desviaciones.

---

**Conclusión Final:**

VitalLunch es un proyecto que tiene sentido. Los números están bien (TIR 100%, VAN S/ 629,707), el modelo aguanta variaciones y los riesgos se pueden manejar con las estrategias que planteamos. No es un proyecto perfecto ni libre de riesgos, pero tiene fundamentos sólidos.

Lo más importante ahora es ejecutar bien, especialmente los primeros 6 meses. Hay que validar rápido si la gente realmente va a pedir con la frecuencia que proyectamos, asegurar los subsidios corporativos y demostrar que podemos entregar a tiempo con calidad constante. Si logramos eso, el resto fluye.

La recomendación es clara: proceder con la inversión, pero con disciplina. Monitorear los números cada mes, ajustar rápido si algo no cuadra y mantener el foco en calidad. Si hacemos las cosas bien, VitalLunch puede convertirse en una opción confiable de alimentación saludable para oficinas en Lima.
